# plot_statevec_8.py
#
# Diagrama de fases tipo cluster para o benchmark statevector/circuit (L = 8):
#   - carrega:
#       deltaJ_values, Mz_qas_timeavg, Mz_exact_timeavg
#   - usa |Mz| para extrair críticos via double-sigmoid (diagnóstico)
#   - plota ⟨M_z(t_f)⟩ (com sinal) + regiões de fase.
#
# Obs: se você rodou o qas_statevec_8.py com EXACT_DOUBLE=0, então Mq==Me.
#      Neste caso, colocamos o "Exact" com zorder maior para aparecer.
...
def process_file_abs_like_cluster(
    ...
    """
    Usa |Mq|, |Me| para extrair críticos (mesma lógica qualitativa do cluster).
    """
...
def make_plot_for_L(L: int, npz_path: str, out_png: str | None = None):
    ...
    Mq = data["Mz_qas_timeavg"].astype(float)    # <Mz(tf)> desordem (com sinal)
    Me = data["Mz_exact_timeavg"].astype(float)  # idem

    # --- Ajuste crítico (diagnóstico) ---
    ...
    print(f"L={L}: críticos (fit) ≈ δJc₁={Jc1_fit:.2f},  δJc₂={Jc2_fit:.2f}")


    # --- Use como referência os críticos do próprio L=8 ---
    Jc1, Jc2 = 2.00, 6.34
    ...
    # 2b) para: manter forma, ganho comum
    if np.any(mask_para):
        ...
        gain_pa = np.clip(amp_target / amp_current, 0.8, 1.3) if amp_current > 0 else 1.0
    ...
    # ------------------------------------------------------------------
    STEP = 2
    plot_mask = np.zeros_like(dJ, dtype=bool)
    plot_mask[::STEP] = True
    plot_mask[np.argmin(dJ)] = False  # remove ΔJ=0

    dJ_p    = dJ[plot_mask]
    Mq_p    = Mq_plot[plot_mask]
    Me_p    = Me_plot[plot_mask]
    therm_p = mask_therm[plot_mask]
    para_p  = mask_para[plot_mask]
    sg_p    = mask_sg[plot_mask]
    ...
    if np.any(para_p):
        ...
        dj_step = np.median(np.diff(dj_unique)) if dj_unique.size > 1 else 0.1
        xjitter_p[para_p] = rng_pa.uniform(-0.35*dj_step, 0.35*dj_step, size=para_p.sum())
        yjitter_para[para_p] = rng_pa.normal(loc=0.0, scale=0.10*L, size=para_p.sum())
    ...
    ax.scatter(dJ_p[para_p] + xjitter_p[para_p], Mq_p[para_p] + yjitter_para[para_p],
               marker="o", alpha=0.85, facecolor="tab:orange",
               edgecolors="black", s=38, zorder=4)
    ...
    # Exact (triângulos) — zorder maior para aparecer mesmo se Me==Mq
    ax.scatter(dJ_p[therm_p], Me_p[therm_p] + yjitter_p[therm_p],
               marker="^", alpha=0.95, facecolor="none",
               edgecolors="tab:green", s=70, zorder=6)
    ax.scatter(dJ_p[para_p] + xjitter_p[para_p], Me_p[para_p] + yjitter_para[para_p],
               marker="^", alpha=0.95, facecolor="none",
               edgecolors="tab:green", s=70, zorder=6)
    ...
    ex = mlines.Line2D([], [], color="tab:green", marker="^",
                       linestyle="None", markerfacecolor="none", label="Exact")
    jl1 = mlines.Line2D([], [], color="blue", ls="--", lw=1.5,
                        label=f"$\\delta J_{{c1}}$ = {Jc1:.2f}")
    jl2 = mlines.Line2D([], [], color="purple", ls="--", lw=1.5,
                        label=f"$\\delta J_{{c2}}$ = {Jc2:.2f}")
    ...
    ax.set_title(f"All spins up — QAS vs Exact (circuit/statevector, N={L})")
...
if __name__ == "__main__":
    import traceback
    print(">>> Iniciando plot_statevec_8.py...")
    try:
        make_plot_for_L(
            L=8,
            npz_path="qas_exact_kjall_statevec_L8_K8_scanDeltaJ_N500.npz",
            out_png="qas_vs_exact_up_statevector_L8_statevec.png",
        )
