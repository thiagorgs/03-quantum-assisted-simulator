...
# ----------------------------------------------------------------------
#  Hamiltoniano de Kjäll et al. em termos de Pauli strings
# ----------------------------------------------------------------------
...
def x_moment_words_from_terms(terms, L: int, K: int):
    """
    Constrói as 'palavras' (tuplas de índices) usando APENAS os termos X_i do Hamiltoniano.
    """
    ...
    max_flips = min(K, L)
    words = [()]  # 0-moment: estado inicial |0...0>
...
# (mantive o k_moment_words aqui caso você queira usar outra topologia de ansatz)
...
# ----------------------------------------------------------------------
#  Matrizes E e D no subespaço CS_K
# ----------------------------------------------------------------------
def compute_E_D(states, terms):
    ...
    # 2) Pré-calcula matrizes dos Pauli strings
    pauli_mats = []
    coeffs = []
    ...
    # 3) D_{ij} = sum_k c_k <psi_i | P_k | psi_j>
    for i in range(m):
        psi_i = states[i].data
        for j in range(m):
            psi_j = states[j].data
...
def qas_disorder_average(
    ...
    """
    Retorna:
      - mags_mean: <Z_i(t)> médio na desordem
      - J_bonds_all: (n_realizations, L-1)
      - Mz_final_all: array com M_z^{(r)}(t_f) para cada realização (com sinal)
    """
...
def exact_Mz_final_disorder_average(
    ...
    """
    Retorna:
      - Mz_exact_mean: <M_z(t_f)> médio na desordem (com sinal)
      - Mz_exact_abs_mean: <|M_z(t_f)|> médio
      - Mz_vals: array com M_z^{(r)}(t_f) para cada realização
    """
...
# ----------------------------------------------------------------------
#  Script principal: gera curva <M_z(t_f)> vs ΔJ e salva em .npz
# ----------------------------------------------------------------------
# %%
if __name__ == "__main__":
    J = 1.0
    J2 = 0.3
    h = 0.6       # h/2 = 0.3, como no artigo

    # grade de tempos (apenas para resolver a EDE no QAS;
    # o observável que vamos guardar é Mz(t_final))
    t_max = 400.0
    ntimes = 201
    t_list = np.linspace(0.0, t_max, ntimes)
    t_final = t_list[-1]

    # parâmetros da desordem
    deltaJ_min = 0.0
    deltaJ_max = 12.0
    deltaJ_step = 0.1
    ...
    n_realizations = 500  # como você já usou

    for L in [6]:
        K = L   # K-moment = número de spins -> espaço de Hilbert completo
        print(f"=== L = {L}, K = {K} ===")

        # vamos guardar <Mz(t_f)> médio na desordem (COM SINAL)
        Mz_qas_vs_dJ   = []
        Mz_exact_vs_dJ = []
        ...
        for idx_dJ, deltaJ in enumerate(deltaJ_values):
            ...
            # --- QAS: <Mz(t_f)> médio na desordem ---
            (
                mags_mean,
                J_bonds_all,
                Mz_final_all,      # M_z^{(r)}(t_f) com sinal, uma entrada por realização
        ...
        # Mantemos os nomes antigos no arquivo, mas AGORA eles significam <Mz(t_f)>_desordem
        np.savez(
            f"qas_exact_kjall_statevec_L{L}_K{K}_scanDeltaJ_N{n_realizations}.npz",
            deltaJ_values=deltaJ_values,
            Mz_qas_timeavg=Mz_qas_vs_dJ,      # <Mz(t_f)>_desordem (com sinal)
            Mz_exact_timeavg=Mz_exact_vs_dJ,  # <Mz(t_f)>_desordem (com sinal)
        )

        # plot de checagem simples (opcional)
        plt.figure()
        plt.plot(deltaJ_values, Mz_qas_vs_dJ,   "o-", label="QAS")
        plt.plot(deltaJ_values, Mz_exact_vs_dJ, "^-", label="Exato")
        plt.xlabel(r"$\Delta J$")
        plt.ylabel(r"$\langle M_z(t_f) \rangle$")
        plt.title(f"L = {L}, K = {K}")
        plt.ylim(-L, L)  # M_z ∈ [-L, +L]
        plt.grid(True)
        plt.legend()
        plt.tight_layout()
        plt.show()
