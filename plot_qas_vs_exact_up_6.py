...
def _phase_lock_should_apply(delta_J: float) -> bool:
    # Caso 1: PHASE_LOCK global e não pediram "SG only" -> sempre liga
    if PHASE_LOCK and not PHASE_LOCK_SG_ONLY:
        return True
    # Caso 2: nada ligado
    if not PHASE_LOCK and not PHASE_LOCK_SG_ONLY:
        return False
    # Caso 3: "SG only" -> precisa de J2_FORCE disponível
    try:
...
def _time_avg_expvals(evals, evecs, coeff0, Obs, times):
    """
    Média temporal de <Obs>(t) com evolução espectral vetorizada.

    evals  : (d,) autovalores (w)
    evecs  : (d,d) autovetores (V)
    coeff0 : (d,) coeficientes no autobase (V^\dagger alpha0 ou V^\dagger psi0)
    Obs    : (d,d) observável na base correta (M_tilde no QAS, M_full no exato)
    times  : (T,) tempos
    """
    # phases: (d, T)
    phases = np.exp(-1j * evals[:, None] * times[None, :])
    # alphas(t) = V @ (phases * coeff0[:,None]) -> (d, T)
    alphas = evecs @ (phases * coeff0[:, None])
    # O_alpha = Obs @ alphas -> (d, T)
    O_alpha = Obs @ alphas
    # <O>(t) = Re[alpha(t)^\dagger O alpha(t)] para cada t
...
def qas_one_mag_qr(delta_J, seed, params, Q, M_tilde, alpha0,
    ...
    t0 = min(Tfin * t0_ratio, Tfin)                 # garante t0 ≤ Tfin
    ...
    if ASSERT_VECT_EQ and not _EQ_DONE_QAS:
        ...
        _EQ_DONE_QAS = True
        if VECT_AVG:
            return val_vec  # já aproveita o valor computado
    if VECT_AVG:
        return _time_avg_expvals(w, V, beta0, M_tilde, times)

    # fallback (antigo loop em t)
    mz_vals = []
    for t in times:
        alpha_t = V @ (np.exp(-1j*w*t) * beta0)
        mz_vals.append(np.real(alpha_t.conj().T @ (M_tilde @ alpha_t)))
...
def process_file_abs_like_cluster(dJ, Mq, Me,
    ...
    if np.count_nonzero(np.isfinite(medians)) < 8:
        # fallback: detecta quebras por máxima derivada de LOWESS “crua”
        xv = centers[valid]; yv = medians[valid]
        # suaviza levemente para derivada estável
        xs, ys = lowess(yv, xv, frac=min(0.25, max(0.1, 8/len(yv))), return_sorted=True).T
        dy = np.gradient(ys, xs)
        i1 = np.nanargmax(dy); i2 = np.nanargmin(dy)
        return {"Jc1": float(xs[i1]), "Jc2": float(xs[i2]), "xfit": xs}
    popt,_ = curve_fit(double_sigmoid, xsm, ysm, p0=p0, bounds=(lb,ub), maxfev=50000)
    return {"Jc1": float(popt[2]), "Jc2": float(popt[6]),
        ...
        for k in range(N_REALIZ):
            ...
            if SYM_UPDOWN:
                mz_qas_dn = qas_one_mag_qr(float(dj), s, params, Q_dn, M_dn, a_dn,
                                        n_time=NTIME_AVG, t0_ratio=T_AVG_START_RATIO)
                # Pelo comutador com X^{⊗N}, vale exatamente: Mz_down_exato = -Mz_up_exato
                mz_ex_dn = -mz_ex
...
def main():
    ...
    agg_path = build_or_load_aggregated_up_sv(OUTDIR, BASE_SEED, params, K=6, N_REALIZ=NREAL)
    ...
    if SYM_UPDOWN and (Mq_all_dn is not None):
        ...
        # combinação antissimétrica: (up - down)/2
        ...
        if Me_all_dn is not None:
            ...
            # se não veio gravado, use a relação exata Me_down = - Me_up
            Me_all[mask_full] = 0.5*(Me_all[mask_full] - (-Me_all[mask_full]))

    # Cópias para plot (sem “buraco”)
    ...
    # Máscara para estatísticas (como no cluster)
    ...
    # Clamp/filtro nas amostras (para as médias por bin)
    ...
    # “Dente-serrilhado” só na fase para
    ...
    # Índices por fase + subsampling visual
