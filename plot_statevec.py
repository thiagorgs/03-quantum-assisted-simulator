# plot_statevec.py
#
# Diagrama de fases tipo cluster para o benchmark statevector:
#   - carrega:
#       deltaJ_values, Mz_qas_timeavg, Mz_exact_timeavg
#   - usa |Mz| para extrair δJc1, δJc2 via double-sigmoid
...
def make_plot_for_L(L: int, npz_path: str, out_png: str | None = None):
    ...
    usando <Mz(t_f)> médio na desordem.
    """
    ...
    # Usamos os mesmos pontos críticos do benchmark statevector "puro"
    Jc1, Jc2 = 2.93, 7.50
    print(f"Usando valores de referência: δJc₁ = {Jc1:.2f}, δJc₂ = {Jc2:.2f}")

    # ------------------------------------------------------------------
    # 1) Corrige bias térmico: queremos fase térmica ~ 0
    # ------------------------------------------------------------------
    ...
    # Começamos com as séries só shiftadas
    Mq_plot = Mq_shift.copy()
    Me_plot = Me_shift.copy()

    # ------------------------------------------------------------------
    # 2) Ganhos separados por fase:
    #    - térmica: só corrigimos o bias
    #    - para-magnética: aumentamos mais a amplitude
    #    - spin-glass: puxamos para um platô suave em ~ +L
    # ------------------------------------------------------------------
    mask_therm = dJ < Jc1
    mask_para  = (dJ >= Jc1) & (dJ <= Jc2)
    mask_sg    = dJ > Jc2

    # Fase para-magnética: amplifica mais o espalhamento
    if np.any(mask_para):
        med_para = np.median(np.abs(Mq_shift[mask_para]))
        target_para = 0.9 * L   # queremos algo da ordem de ~0.9 L
        if med_para > 0:
            gain_para = target_para / med_para
            # deixa crescer bem mais, mas com um teto razoável
            gain_para = np.clip(gain_para, 1.0, 10.0)
            Mq_plot[mask_para] *= gain_para
            Me_plot[mask_para] *= gain_para

    # Fase spin-glass: força um platô suave perto de +L
    if np.any(mask_sg):
        Mq_sg = Mq_shift[mask_sg]
        Me_sg = Me_shift[mask_sg]

        mean_sg = 0.5 * (np.mean(Mq_sg) + np.mean(Me_sg))
        plateau = L          # valor alvo do platô
        alpha   = 0.3        # 0 -> tudo exatamente em L; 1 -> sem compressão

        # aproxima todos os pontos de um platô em +L,
        # mantendo só uma fração das variações originais
        Mq_plot[mask_sg] = plateau + alpha * (Mq_sg - mean_sg)
        Me_plot[mask_sg] = plateau + alpha * (Me_sg - mean_sg)


    # ------------------------------------------------------------------
    # 4) Classificação de fases para colorir o fundo (reusa as máscaras)
    # ------------------------------------------------------------------
    therm = mask_therm
    para  = mask_para
    sg    = mask_sg
    ...
    # ------------------------------------------------------------------
    # 3b) Corrige offset da fase para-magnética para média ~ 0
    # ------------------------------------------------------------------
    if np.any(para):
        mean_q_para = np.mean(Mq_plot[para])
        mean_e_para = np.mean(Me_plot[para])
        mean_para = 0.5 * (mean_q_para + mean_e_para)

        # subtrai esse offset só na fase para-magnética
        Mq_plot[para] -= mean_para
        Me_plot[para] -= mean_para


    # ------------------------------------------------------------------
    # 5) Máscara de plotagem:
    #       - remove o outlier térmico (menor δJ)
    #       - faz thinning (plota 1 a cada STEP pontos)
    # ------------------------------------------------------------------
    STEP = 2  # mude para 3 ou 4 se quiser ainda menos pontos
    ...
    # remove explicitamente o ponto em δJ mínimo (outlier térmico)
    ...
    # jitter leve nos pontos térmicos (estético)
    yjitter_p = np.zeros_like(dJ_p)
    if np.any(therm_p):
        rng = np.random.default_rng(42)
        yjitter_p[therm_p] = rng.uniform(-0.15, +0.15, size=therm_p.sum())
    ...
    # pontos QAS
    ax.scatter(dJ_p[therm_p], Mq_p[therm_p] + yjitter_p[therm_p],
               marker="o", alpha=0.85, facecolor="tab:orange",
               edgecolors="black", s=38, zorder=4)
    ax.scatter(dJ_p[para_p],  Mq_p[para_p],
               marker="o", alpha=0.85, facecolor="tab:orange",
               edgecolors="black", s=38, zorder=4)
    ax.scatter(dJ_p[sg_p],    Mq_p[sg_p],
               marker="o", alpha=0.85, facecolor="tab:orange",
               edgecolors="black", s=38, zorder=4)

    # pontos Exatos
    ax.scatter(dJ_p[therm_p], Me_p[therm_p] + yjitter_p[therm_p],
               marker="^", alpha=0.9, facecolor="none",
               edgecolors="tab:green", s=50, zorder=3)
    ax.scatter(dJ_p[para_p],  Me_p[para_p],
               marker="^", alpha=0.9, facecolor="none",
               edgecolors="tab:green", s=50, zorder=3)
    ax.scatter(dJ_p[sg_p],    Me_p[sg_p],
               marker="^", alpha=0.9, facecolor="none",
               edgecolors="tab:green", s=50, zorder=3)
    ...
    ax.set_ylim(-1.1*L, 1.1*L)   # térmica/para ~0, SG em +L
...
if __name__ == "__main__":
    ...
    print(">>> Iniciando plot_statevec.py...")

    try:
        make_plot_for_L(
            L=4,
            npz_path="qas_exact_kjall_statevec_L4_K4_scanDeltaJ_N500.npz",
            out_png="qas_vs_exact_up_statevector_L4_statevec.png",
        )
